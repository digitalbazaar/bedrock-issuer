  ...
  async.auto({
    ...
    issue: ['sign', function(callback, results) {
      ...
      store.provider.insert(
        actor, results.sign, opts, function(err, record) {
  ...
  async.auto({
    checkPermission: function(callback) {
      brPermission.checkPermission(
        ...
        {resource: id, translate: 'claim'}, callback);
    },
...
function getCredentialSigningKey(credential, callback) {
  async.auto({
    ...
    getPreferredSigningKey: ['getIssuer', function(callback, results) {
      ...
      brKey.getPublicKey(
        {id: keyId}, identity, (err, publicKey, meta, privateKey) => {
          if(err) {
            return callback(new BedrockError(
...
var CREDENTIAL_FRAME = {
  ...
  'https://w3id.org/credentials#issuer': {
    '@omitDefault': true,
    '@embed': false
  }
};
    ...
    function(req, res, next) {
      ...
      async.auto({
        ...
        issue: ['generateId', 'validated', function(callback, results) {
          ...
          api.issue(
            req.user.identity, credential, options, function(err, issued) {
              if(err) {
