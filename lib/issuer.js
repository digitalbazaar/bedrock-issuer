...
api.revoke = function(actor, id, options, callback) {
  async.auto({
    ...
    getSigningKey: ['get', function(callback) {
...
function getCredentialSigningKey(credential, callback) {
  async.auto({
    ...
    getPreferredSigningKey: ['getIssuer', function(callback, results) {
      ...
      if(identity.sysPreferences && identity.sysPreferences.credentialSigningKey) {
    ...
    if(!results.getPreferredSigningKey) {
      return callback(new BedrockError(
        'Preferred credential signing key not found.',
        'NotFound',
