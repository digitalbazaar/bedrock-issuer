...
function checkHeaders(req, res, next) {
  ...
  if(req.get('Content-Type').indexOf(';') === -1) {
    contentType = req.get('Content-Type');
  } else {
    contentType = req.get('Content-Type').match(/^(.*?);/)[1];
  }
...
bedrock.events.on('bedrock-express.configure.routes', function(app) {
  ...
  app.post(
    ...
    function(req, res, next) {
      ...
      var options = {};
      options.privateKey = config.issuer.credentialSigningKeys.privateKeyPem;
      options.publicKeyId = config.issuer.credentialSigningKeys.publicKeyId;
      var credential = req.body;
      // FIXME: null should be replaced with actor
  ...
  app.post(
    ...
    function(req, res, next) {
      ...
      async.waterfall([
        function(callback) {
          jsigs.sign(credential, {
            privateKeyPem: privateKeyPem, creator: publicKeyId
          }, callback);
        },
  ...
  app.get(config.issuer.endpoints.signingKeys, function(req, res, next) {
    var keyInfo = {
      '@context': 'https://w3id.org/identity/v1',
      id: config.issuer.credentialSigningKeys.publicKeyId,
      owner: config.server.baseUri,
  ...
  app.get('/', function(req, res, next) {
    var acceptableContentTypes = ['text/html', 'application/ld+json'];
